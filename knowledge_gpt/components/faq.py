...
def faq():
    st.markdown(
        """
# FAQ
## How does DocChaineGPT by syst work?
...
When you ask a question, DCGT will search through the
document chunks and find the most relevant ones using the vector index.
Then, it will use GPT3 to generate a final answer.

## What is Used in this build ?
1. SYST LLM (SlliaAM 1.1)
2. OpenAI LLM (ChatGPT 3)

## Is my data safe?
Yes, your data is safe. DCGPT does not store your documents or
...
No, the answers are not 100% accurate. DCGPT uses GPT-3 to generate
answers. GPT-3 is a powerful language model, but it sometimes makes mistakes 
and is prone to hallucinations. Also, DCGPT uses semantic search
...
But for most use cases, DCGPT is very accurate and can answer
